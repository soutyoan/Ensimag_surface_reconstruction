...
int Bloomenthal::triproc(int i1, int i2, int i3, VERTICES vertices)
{
  ...
  if(gntris%100 == 0)
    printf("\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b  %d tiriangles", gntris);
...
char* Bloomenthal::polygonize(Mesh* mesh, double size, float bounds[6], double x, double y, double z, int mode)
{
  ...
  printf("\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b %d tiriangles\n", gntris);
...
Bloomenthal::TEST1* Bloomenthal::find1(int sign1, PROCESS *p, double x, double y, double z)
{
  int i;
  ...
  for (i = 0; i < 100000; i++) {
    ...
    if (sign1 == (test->value > 0.0)) return test;
...
{
  return func->value((float)x, (float)y, (float)z);
}
